            ...
            <div className="todo flex flex-col  p-3  ">
                ...
                <div className="flex flex-col gap-y-4 mt-4 h-100 overflow-scroll">
                    {Tasks && Tasks.filter(task => task.state === "todo").map(task => {
                        return (
                            <div className="hover:drop-shadow-2xl w-auto h-auto bg-slate-800 rounded-lg p-6 text-white  hover:border-indigo-600 hover:border-2" key={task._id}>
            ...
            <div className="todo flex flex-col  p-3">
                ...
                <div className="flex flex-col gap-y-4 mt-4 h-100 overflow-scroll">
                    {Tasks && Tasks.filter(task => task.state === "done").map(task => {
                        return (
                            <div className="hover:drop-shadow-2xl w-auto h-auto bg-slate-800 rounded-lg p-6 text-white  inline hover:border-indigo-600 hover:border-2 " key={task._id}>
                                <h1 className="text-xl inline">{task.title}</h1>
                                {activeGroup === "main" &&
                                    <h2 className='inline ml-2 text-xs text-gray-600'>{task.group.name}</h2>
                                }
                                <p className="text-sm py-4">{task.body} </p>
                                {activeGroup === "main" &&
                                    <button className='block text-sm bg-indigo-700 p-1 px-2 rounded-2xl hover:bg-indigo-800 hover:shadow-lg active:bg-red-800  active:text-white transition-colors delay-50' onClick={() => deleteTask(task._id)}>Delete</button>
                                }
                            </div>)
                    })}
